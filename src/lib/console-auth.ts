...
/**
 * console-auth.ts â€” THE ENTRY GATE FOR EVERY `/api/console/*` ROUTE.
 *
 * ============================================================================
 ...
 * There is no database net below the application layer â€” this guard IS the
 * tenant boundary.
 *
 ...
 *   >> read from the body, the query string, or the URL path â€” with the single
 ...
 *
 * ============================================================================
 *  CORS â€” CONSOLE ROUTES ARE SAME-ORIGIN ONLY
 * ============================================================================
...
/**
 * NOTE ON SHAPE: flat optional fields, NOT a discriminated union â€” identical
 ...
 *
 * Usage â€” the first two lines of every console handler, no exceptions:
 *
 ...
 *
 * ============================================================================
 *  TIER ENFORCEMENT IS BUILT IN â€” DO NOT ADD IT PER-ROUTE
 * ============================================================================
 ...
 * that every route must already pass through cannot be forgotten â€” a new
 ...
 *                       `?client_id=` override â€” never to scope a customer.
...
export async function requireConsoleSession(
  ...
  // value is only ever compared against their own id â€” never substituted for it.
  ...
  // TIER CHECK â€” after authentication and tenant resolution, never before.
  //
  ...
  // doing support, not a customer consuming a feature â€” and billing us for our
 ...
 * get a rendered shell â€” even an empty one leaks the other company's branding,
...
export type ConsoleAccess = {
   ...
   * PAYING customer to `/upvc/login` is a terrible experience â€” they are logged in;
...
export async function requireConsoleAccess(resolvedClientId: string): Promise<ConsoleAccess> {
  const session = await getSession();
  if (!session) return { ok: false, redirectTo: "/upvc/login" };
  ...
  if (session.role === "customer") {
    ...
    // Not `notFound()` â€” a 404 vs a redirect is itself an oracle telling the
    // caller whether the other tenant's slug exists.
    if (!own || own !== resolvedClientId) return { ok: false, redirectTo: "/upvc/login" };
    ...
    //
    // This is UX, NOT security â€” `requireConsoleSession()` on each API route is
  ...
  return { ok: false, redirectTo: "/upvc/login" };
}
