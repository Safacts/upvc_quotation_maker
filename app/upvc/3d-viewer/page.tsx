...
export default function Page({ searchParams }) {
  ...
  useEffect(() => {
    const designId = searchParams && searchParams.designId;
    const fromQuotation = searchParams && searchParams.fromQuotation;
    ...
    const load = async () => {
      try {
        if (designId) {
          ...
          // Pull the quotation, take the first measured opening, and run it
          // through the configurator so the live design matches the saved
          // dimensions instead of a hand-built shape.
          ...
          const measured = qData.measured_items || [];
          const first = measured.find((m) => Number(m.width) > 0 && Number(m.height) > 0) || measured[0];
          if (!first) throw new Error('quotation has no measured openings');
          ...
          const cfgRes = await fetch('/api/console/3d/configurator', {
            ...
            credentials: 'include',
            body: JSON.stringify({
              width_mm: Number(first.width),
              height_mm: Number(first.height),
              type: 'fixed',
              name: `Quotation ${q.quote_no || fromQuotation}`,
            }),
          });
          const cfgData = await cfgRes.json();
          if (!cfgRes.ok) throw new Error((cfgData && cfgData.error) || 'configurator failed');
          setDesign(cfgData.design);
        }
      } catch {
        // Same silent fallback as the designId path: the viewer shows the empty
        // state rather than throwing a wall of text.
      } finally {
        setLoading(false);
      }
    };
    void load();
  }, [searchParams && searchParams.designId, searchParams && searchParams.fromQuotation]);
