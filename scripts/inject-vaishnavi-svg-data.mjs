...
const source = resolve(arg("--source", "src/templates/vaishnavi/page1.svg"));
const payloadPath = resolve(arg("--payload", String.raw`C:\Users\aadi\Downloads\kpr-vaishnavi-payload.json`));
const output = resolve(arg("--output", String.raw`C:\Users\aadi\Downloads\KPRUPVC-07082026-0153-Vaishnavi-Injected.svg`));
...
const indian = (value) => Number(value).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
const decimal = (value) => Number.isInteger(Number(value)) ? String(Number(value)) : Number(value).toFixed(1);
...
function replaceNear(targetY, targetX, value, toleranceX = 1, fitWidth = null, fontFamily = "Arial, sans-serif", eraseDistance = null) {
  let replaced = false;
  svg = svg.replace(/<tspan y="([0-9.]+)" x="([^"]+)"([^>]*)>([\s\S]*?)<\/tspan>/g, (whole, y, xs, rest) => {
    ...
    if (!replaced && Math.abs(x - targetX) <= toleranceX) {
      ...
      const fit = fitWidth && estimatedWidth > fitWidth
        ? ` textLength="${fitWidth}" lengthAdjust="spacingAndGlyphs"`
        : "";
      // The source PDF-to-SVG contains one x coordinate per original glyph. If
      // we keep that array while changing the text, the new glyphs inherit stale
      // positions and visibly collide. Re-anchor the replacement as a normal
      // text run; textLength then fits that run into the original cell width.
      return `<tspan y="${y}" x="${targetX}" font-family="${fontFamily}"${cleanRest}${fit}>${escapeXml(value)}</tspan>`;
    }
    // PDF converters can leave adjacent fragments from the old text. Clear
    // only fragments on this baseline and inside this field's original width.
